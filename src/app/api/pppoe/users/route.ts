  ...
  try {
    ...
    if (username || password || profileId || ipAddress || routerId !== undefined) {
      try {
        ...
        // If profile changed, apply CoA to active session
        const profileChanged = profileId && profileId !== currentUser.profileId;
        if (profileChanged && newProfile) {
          console.log(`[User Update] Profile changed for user ${newUsername}, applying CoA...`);
          ...
          if (coaAvailable) {
            ...
            if (activeSession) {
              const coaResult = await applyProfileChangeToActiveSessions(
                newUsername,
                [{
                ...
                {
                  downloadSpeed: newProfile.downloadSpeed,
                  uploadSpeed: newProfile.uploadSpeed,
                  groupName: newProfile.groupName,
                },
                { fallbackToDisconnect: true }
              );

              console.log(`[User Update] CoA result for ${newUsername}:`, coaResult);
