  ...
  return (
    <ResponsiveContainer width="100%" height={height}>
      <BarChart data={data} margin={{ top: 5, right: 5, left: -15, bottom: 0 }}>
        <CartesianGrid strokeDasharray="3 3" stroke="#e5e7eb" vertical={false} />
        <XAxis dataKey={nameKey} tick={{ fontSize: 9 }} tickLine={false} axisLine={false} angle={-15} textAnchor="end" />
        <YAxis tickFormatter={formatCurrency} tick={{ fontSize: 10 }} tickLine={false} axisLine={false} />
        <Tooltip content={<CustomTooltip valuePrefix="Rp " />} />
  ...
  return (
    <ResponsiveContainer width="100%" height={height}>
      <PieChart>
        ...
        <Tooltip formatter={(value: any) => [value ? Number(value).toLocaleString('id-ID') : '0', 'User']} />
      </PieChart>
    </ResponsiveContainer>
  );
}
  ...
  return (
    <ResponsiveContainer width="100%" height={height}>
      <PieChart>
        ...
        <Tooltip formatter={(value: any) => [value ? Number(value).toLocaleString('id-ID') : '0', 'Voucher']} />
        <Legend 
          layout="horizontal" 
  ...
  return (
    <ResponsiveContainer width="100%" height={height}>
      <AreaChart data={data} margin={{ top: 5, right: 5, left: -15, bottom: 0 }}>
        ...
        <YAxis tick={{ fontSize: 10 }} tickLine={false} axisLine={false} />
        <Tooltip content={<CustomTooltip />} />
        <Area type="monotone" dataKey="pppoe" stroke={COLORS.primary} fill="url(#pppoeGradient)" strokeWidth={2} name="PPPoE" />
        <Area type="monotone" dataKey="hotspot" stroke={COLORS.warning} fill="url(#hotspotGradient)" strokeWidth={2} name="Hotspot" />
  ...
  return (
    <ResponsiveContainer width="100%" height={height}>
      <AreaChart data={data} margin={{ top: 5, right: 5, left: -15, bottom: 0 }}>
        ...
        <Tooltip formatter={(value: any) => [value ? formatBandwidth(Number(value)) : '0 MB', '']} />
        <Area type="monotone" dataKey="upload" stroke={COLORS.success} fill="url(#uploadGradient)" strokeWidth={2} name="Upload" />
        <Area type="monotone" dataKey="download" stroke={COLORS.info} fill="url(#downloadGradient)" strokeWidth={2} name="Download" />
        <Legend wrapperStyle={{ fontSize: '10px' }} />
      </AreaChart>
    </ResponsiveContainer>
  );
        ...
        return (
          <div key={index} className="space-y-1">
            <div className="flex justify-between items-center text-xs">
              <span className="text-gray-700 dark:text-gray-300 truncate flex-1">{item.source}</span>
